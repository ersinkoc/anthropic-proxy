...
type ConfigManager struct {
	...
	mu          sync.RWMutex
	cfg         *Config
	dotEnvMTime time.Time
}

func parseDotEnv(path string) (map[string]string, time.Time, error) {
	...
	return envs, info.ModTime(), nil
}

func envOr(fileEnv map[string]string, k, d string) string {
	if v := os.Getenv(k); v != "" {
		return v
	}
	if v := fileEnv[k]; v != "" {
...
func firstNonEmpty(fileEnv map[string]string, k string) (string, bool) {
	if v := os.Getenv(k); v != "" {
		return v, true
	}
	if v := fileEnv[k]; v != "" {
...
func envBool(fileEnv map[string]string, k string, d bool) bool {
	v, ok := firstNonEmpty(fileEnv, k)
	if !ok {
		return d
	}
...
func envInt(fileEnv map[string]string, k string, d int) int {
	v, ok := firstNonEmpty(fileEnv, k)
	if !ok {
		return d
	}
...
func loadConfig(fileEnv map[string]string) (*Config, error) {
	cfg := &Config{
		ListenAddr:        envOr(fileEnv, "LISTEN_ADDR", ":8787"),
		UpstreamURL:       envOr(fileEnv, "UPSTREAM_URL", "https://api.openai.com/v1/chat/completions"),
		UpstreamKey:       envOr(fileEnv, "UPSTREAM_API_KEY", ""),
		DefaultModel:      envOr(fileEnv, "DEFAULT_MODEL", ""),
		ForceModel:        envBool(fileEnv, "FORCE_MODEL", true),
	...
	if cfg.UpstreamKey == "" {
		return nil, errors.New("UPSTREAM_API_KEY is required")
	}

	if raw := envOr(fileEnv, "MODEL_MAP", ""); raw != "" {
		m := map[string]string{}
	...
	if cfg.ForceModel && cfg.DefaultModel == "" {
		return nil, errors.New("DEFAULT_MODEL is required when FORCE_MODEL=1")
	}
	if !cfg.ForceModel && cfg.DefaultModel == "" && len(cfg.ModelMap) == 0 {
		return nil, errors.New("define DEFAULT_MODEL or MODEL_MAP")
	}
	return cfg, nil
}

func loadConfigFromPath(path string) (*Config, time.Time, error) {
	fileEnv, modTime, err := parseDotEnv(path)
	if err != nil {
		return nil, time.Time{}, err
	}
	cfg, err := loadConfig(fileEnv)
	if err != nil {
		return nil, time.Time{}, err
	}
...
func NewConfigManager(path string) (*ConfigManager, error) {
	cfg, modTime, err := loadConfigFromPath(path)
	if err != nil {
		return nil, err
	}
...
func (m *ConfigManager) maybeReload() error {
	fileEnv, modTime, err := parseDotEnv(m.path)
	if err != nil {
		return err
	}

	m.mu.RLock()
	currentMod := m.dotEnvMTime
	currentCfg := m.cfg
	m.mu.RUnlock()
	if modTime.Equal(currentMod) {
		return nil
	}
	...
	m.mu.Lock()
	defer m.mu.Unlock()
	if modTime.Equal(m.dotEnvMTime) {
		return nil
	}
	listenChanged := currentCfg != nil && currentCfg.ListenAddr != cfg.ListenAddr
	m.cfg = cfg
	m.dotEnvMTime = modTime
	if listenChanged {
		log.Printf("config reloaded from %s (LISTEN_ADDR changed; restart required to apply)", m.path)
		return nil
	}
...
func (p *Proxy) convertRequest(a *AnthropicRequest, cfg *Config) (*OpenAIRequest, error) {
	o := &OpenAIRequest{
		Model:       p.mapModel(cfg, a.Model),
...
func (p *Proxy) currentConfig() *Config {
	return p.cfgManager.Current()
}

func (p *Proxy) mapModel(cfg *Config, anthropicModel string) string {
	if cfg.ForceModel && cfg.DefaultModel != "" {
		return cfg.DefaultModel
	}
	if v, ok := cfg.ModelMap[anthropicModel]; ok {
		return v
	}
...
func (p *Proxy) checkAuth(r *http.Request, cfg *Config) bool {
	...
	if k := r.Header.Get("x-api-key"); k == cfg.ExpectedClientKey {
		return true
	}
	if auth := r.Header.Get("Authorization"); auth != "" {
		if strings.TrimPrefix(auth, "Bearer ") == cfg.ExpectedClientKey {
			return true
		}
	}
	return false
...
func (p *Proxy) handleMessages(w http.ResponseWriter, r *http.Request) {
	...
	oreq, err := p.convertRequest(&areq, cfg)
	if err != nil {
		writeAnthropicError(w, http.StatusBadRequest, "invalid_request_error", err.Error())
		return
	}
	...
	if areq.Stream {
		p.handleStream(w, r.Context(), cfg, &areq, oreq, reqID)
		return
	}
	p.handleSync(w, r.Context(), cfg, &areq, oreq, reqID)
}
...
func (p *Proxy) handleSync(w http.ResponseWriter, ctx context.Context, cfg *Config, areq *AnthropicRequest, oreq *OpenAIRequest, reqID uint64) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, "POST", cfg.UpstreamURL, bytes.NewReader(reqBytes))
	if err != nil {
		writeAnthropicError(w, http.StatusInternalServerError, "api_error", err.Error())
		return
	}
	httpReq.Header.Set("Content-Type", "application/json")
	httpReq.Header.Set("Authorization", "Bearer "+cfg.UpstreamKey)
...
func (p *Proxy) routes() *http.ServeMux {
	...
	mux.HandleFunc("GET /", func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"models":              cfg.ModelMap,
			"request_timeout_sec": int(cfg.RequestTimeout / time.Second),
		})
	})
	return mux
}
...
func logStartupConfig(cfg *Config) {
	...
	log.Printf("  force    : %v", cfg.ForceModel)
